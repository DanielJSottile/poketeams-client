const POKEMON = require('./newPokemon');
...
function removeWhiteSpaceHyphen(string) {
  return string.replace(/-|\s/g,"");
}

function isLegalSpecies(species){
  return POKEMON.has(removeWhiteSpaceHyphen(species));
};

function findSpecies(species){
  if(isLegalSpecies(species)){

    return POKEMON.get(removeWhiteSpaceHyphen(species))
  } 
};
