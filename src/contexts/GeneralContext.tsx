...
export const GeneralProvider = ({ children }: Props) => {
  ...
  useEffect(() => {
    if (TokenService.getAuthToken()) {
      ...
      apiService
        .getUserTeams(user_id) // Get Public Teams First
        .then((data) => {
          setState((oldVals) => ({ ...oldVals, userTeams: data }));
        });

      apiService
        .getUserSets(user_id) // Get Public Teams First
        .then((data) => {
    ...
    apiService.getTenTeamsSearch(query).then((teams) => {
      ...
      teams.forEach((team: any) => {
        // now we need to get the sets for those 10 teams and put them in state.
